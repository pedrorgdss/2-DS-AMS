...
fun DiceWithButtonAndImage(
    ...
    var numeroUsuario by remember { mutableStateOf<Int?>(null) }
    ...
    Column(
        ...
        Button(onClick = {
            ...
            if (numeroUsuario != null) {
                if (numeroUsuario == result) {
                    Toast.makeText(context, "Acertou!", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, "Errou! Saiu $result", Toast.LENGTH_SHORT).show()
                }
            } else {
    ...
    // 🔹 Dialog simples (só para guardar número)
    if (showDialog) {
        AlertDialog(
            onDismissRequest = {},
            title = { Text("Digite um número (1 a 6)") },
            text = {
                OutlinedTextField(
                    value = userInput,
